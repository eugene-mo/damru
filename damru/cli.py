...
def _start_docker_lines(sudo: str = "", attempts: int = 60) -> list[str]:
    ...
    return [
        *_preferred_iptables_backend_lines(sudo),
        ...
        f"if ! docker info >/dev/null 2>/dev/null; then {prefix}service docker start 2>/dev/null || true; fi",
        "if ! docker info >/dev/null 2>/dev/null; then",
    ...
    if _is_windows():
        ...
        script_lines = [
            ...
            *_restart_docker_lines("sudo_cmd"),
            ...
            "if ! docker info >/dev/null 2>/dev/null; then sudo_cmd service docker start 2>/dev/null || true; fi",
            "if ! docker info >/dev/null 2>/dev/null; then",
